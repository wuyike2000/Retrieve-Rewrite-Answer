...
train=pd.read_table('../../corpus_generation/MetaQA/processed_data/train.txt', header=None, names=["question", "entity_name", "relation", "object",'graph'])
valid=pd.read_table('../../corpus_generation/MetaQA/processed_data/dev.txt', header=None, names=["question", "entity_name", "relation", "object",'graph'])
test=pd.read_table('../../corpus_generation/MetaQA/processed_data/test.txt', header=None, names=["question", "entity_name", "relation", "object",'graph'])
...
text=[]
label=[]
for q,r in zip(train['question'],train['relation']):    
    chain=r.split('|')
    text.append(q)
    label.append(redict[chain[0]])
    text.append(q+'|'+chain[0])
...
text=[]
label=[]
for q,r in zip(valid['question'],valid['relation']):    
    chain=r.split('|')
    text.append(q)
    label.append(redict[chain[0]])
    text.append(q+'|'+chain[0])
...
text=[]
label=[]
for q,r in zip(test['question'],test['relation']):    
    chain=r.split('|')
    text.append(q)
    label.append(redict[chain[0]])
    text.append(q+'|'+chain[0])
